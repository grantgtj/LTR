...
def classify_sum(val: float) -> str:
...
def parse_csv(filepath: str) -> dict:
    """
    Returns a dict: { job_name -> {"prefix": str, "sum": float, "status": str|None} }
    ...
    with open(filepath, newline="", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            ...
            try:
                total = float(row.get("Sum", -1))
            except ValueError:
                total = -1.0
            status = classify_sum(total)
            records[job] = {"prefix": prefix, "sum": total, "status": status}
    return records
...
def compare(file1: str, file2: str, outfile: str):
    ...
    # Head-to-head tally (both executed)
